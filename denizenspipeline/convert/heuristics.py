...
Heuristics live in a directory (default: ``~/.denizens/heuristics/``,
overridable via ``DENIZENS_HEURISTICS_DIR``).  Each heuristic is a Python
...
from denizenspipeline.convert.errors import HeuristicError
from denizenspipeline.convert.manifest import HeuristicRef, ScannerInfo
...
def _heuristics_dir() -> Path:
    ...
    Override with the ``DENIZENS_HEURISTICS_DIR`` environment variable.
    """
    d = Path(os.environ.get("DENIZENS_HEURISTICS_DIR", str(DEFAULT_HEURISTICS_DIR)))
