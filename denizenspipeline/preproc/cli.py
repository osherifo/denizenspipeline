"""CLI subcommands for ``denizens preproc``.

Commands:
    denizens preproc run      — run preprocessing via a backend
    denizens preproc collect   — build manifest from existing outputs
    denizens preproc validate  — validate a manifest (optionally against a config)
    denizens preproc info      — display manifest details
...
def add_preproc_subcommands(subparsers: argparse._SubParsersAction) -> None:
    """Register ``denizens preproc`` and its sub-subcommands."""
...
def dispatch_preproc(args) -> int:
    ...
    if cmd == "run":
        ...
        print("Usage: denizens preproc {run|collect|validate|info|doctor}")
        return 1
...
def _preproc_run(args) -> int:
    from denizenspipeline.preproc.manifest import PreprocConfig, ConfoundsConfig
    from denizenspipeline.preproc.runner import run_preprocessing
...
def _preproc_collect(args) -> int:
    from denizenspipeline.preproc.manifest import PreprocConfig
    from denizenspipeline.preproc.runner import collect_outputs
...
def _preproc_validate(args) -> int:
    ...
    from denizenspipeline.preproc.validation import validate_manifest
...
def _preproc_doctor(args) -> int:
    from denizenspipeline.preproc.backends import list_backends, get_backend
    from denizenspipeline.preproc.manifest import PreprocConfig
