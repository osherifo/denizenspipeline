...
def test_precomputed_subject_is_staged_as_a_copy(tmp_path):
    root = tmp_path / "fs"; _recon(root, "ANfs"); (root / "fsaverage").mkdir()
    bids = tmp_path / "bids"; bids.mkdir()
    work = tmp_path / "work"
    node = FmriprepNode()
    inputs = {"bids_dir": str(bids), "subject": "AN", "output_dir": str(tmp_path / "out"), "work_dir": str(work), "fs_subjects_dir": str(root)}
    params = {"mode": "func_precomputed_anat", "fs_subject": "ANfs"}
    ...
    assert (staged / "sub-AN" / "surf" / "lh.white").is_file() and not (staged / "sub-AN").is_symlink()
    assert not (staged / "sub-AN" / "scripts" / "IsRunning.lh+rh").exists()      # stale lock dropped
    assert (staged / "fsaverage").is_symlink()
    assert (staged / "sub-AN" / ".fmriflow_staged_from").read_text() == str((root / "ANfs").resolve())
    # writing into the copy leaves the original alone
    (staged / "sub-AN" / "surf" / "lh.thickness").write_bytes(b"new")
    assert not (root / "ANfs" / "surf" / "lh.thickness").exists()
    # a second build reuses the staged copy instead of re-copying
    (staged / "sub-AN" / "marker").write_text("keep")
    node.build_command(inputs, params, tmp_path / "node")
    assert (staged / "sub-AN" / "marker").exists()
    # the checkpoint context points at the staged subject
    ctx = node.checkpoint_context(inputs, params, tmp_path / "node")
    assert ctx["fs_subject_dir"] == str(staged / "sub-AN")


def test_missing_or_incomplete_precomputed_subject_fails_fast(tmp_path):
    root = tmp_path / "fs"; _recon(root, "ANfs"); (root / "sub-01").mkdir()
    bids = tmp_path / "bids"; bids.mkdir()
    node = FmriprepNode()
    inputs = {"bids_dir": str(bids), "subject": "AN", "fs_subjects_dir": str(root)}
    errs = node.validate(inputs, {"mode": "func_precomputed_anat"})            # expects sub-AN, absent
    assert any("no precomputed FreeSurfer subject 'sub-AN'" in e and "ANfs" in e for e in errs), errs
    ...
    errs = node.validate(inputs, {"mode": "func_precomputed_anat", "fs_subject": "ANfs"})
