    ...
    (tmp_path / "my_smooth.py").write_text(textwrap.dedent('''
        ...
        class BrandNew:
            ...
            def run(self, inputs, out_dir, params):
                return {}
    '''))
    reg = NodeRegistry(include_parked=True, user_dirs=[PARKED_DIR, tmp_path]).discover()
...
def test_parked_nodes_are_not_in_the_default_library():
    ...
    env = {k: v for k, v in os.environ.items() if k != "FMRIFLOW_INCLUDE_PARKED_NODES"}
    ...
    assert names == {"fmriprep", "bids_source", "derivatives_source", "physio_regressors", "physio_clean"}
    assert not {"bids_app", "custom_shell", "identity", "mask_apply", "reference_fsl_ants", "select"} & names


def test_parked_names_come_back_with_the_flag():
    from fmriflow.preproc.node_registry import PARKED_NODE_NAMES
    assert PARKED_NODE_NAMES == {"smooth", "regress_confounds", "physio_estimate", "manifest_source"}
    assert PARKED_NODE_NAMES <= set(NodeRegistry(include_parked=True, user_dirs=[]).discover().names())
    assert not PARKED_NODE_NAMES & set(NodeRegistry(include_parked=False, user_dirs=[]).discover().names())
...
def test_same_file_loaded_twice_is_not_a_shadow(tmp_path, caplog):
    (tmp_path / "dup.py").write_text(textwrap.dedent('''
        ...
        class Dup:
            ...
            def run(self, i, o, p): return {}
    '''))
    NodeRegistry(include_parked=True, user_dirs=[PARKED_DIR, tmp_path]).discover()
