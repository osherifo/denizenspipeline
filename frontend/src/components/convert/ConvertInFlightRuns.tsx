/** In-flight panel for convert runs — watch log, cancel, reattach status. */
import { useEffect, useState } from 'react'
import type { CSSProperties } from 'react'
import type { ConvertRunSummary } from '../../api/types'
import {
  fetchConvertRuns,
  fetchConvertRun,
  cancelConvertRun,
...
export function ConvertInFlightRuns() {
  const [runs, setRuns] = useState<ConvertRunSummary[]>([])
  ...
  async function openLog(runId: string) {
    ...
    setLogLoading(true)
    try {
      const detail = await fetchConvertRun(runId)
      setLogDetail(detail)
    } catch (e) {
