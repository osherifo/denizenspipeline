/** Sidebar: user module list + template picker. */
import { useEffect, useState } from 'react'
import type { CSSProperties } from 'react'
import type { UserModule } from '../../api/types'
...
const moduleItem = (active: boolean): CSSProperties => ({
  padding: '8px 16px',
  fontSize: 13,
  cursor: 'pointer',
  display: 'flex',
...
export function ModuleSidebar({
  ...
  return (
    <div style={sidebarStyle}>
      <div style={sectionTitle}>My Modules</div>
      <div style={{ flex: 1, overflowY: 'auto' }}>
        {userModules.map((p) => (
          <div
            key={p.name}
            style={moduleItem(currentName === p.name)}
      ...
      <div style={{ borderTop: '1px solid var(--border)' }}>
        ...
        >
          New Module {showTemplates && <span style={{ fontSize: 10, fontWeight: 400 }}>&#x2715;</span>}
        </div>
        {!showTemplates ? (
