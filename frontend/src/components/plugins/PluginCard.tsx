...
export function PluginCard({ plugin }: PluginCardProps) {
  ...
  return (
    ...
    >
      ...
      <div>
        <span style={categoryBadgeStyle}>{plugin.category}</span>
        {plugin.n_dims != null && (
          <span style={dimsBadgeStyle}>{plugin.n_dims} dims</span>
        )}
      </div>
      <div style={docStyle}>{plugin.docstring}</div>
      ...
      {expanded && <ParamTable params={plugin.params} />}
    </div>
  )
}
