/** In-flight panel for autoflatten runs — watch log, cancel, reattach status. */
import { useEffect, useState } from 'react'
import type { CSSProperties } from 'react'
import type { AutoflattenRunSummary } from '../../api/types'
import {
...
function LogModal({
  ...
  detail: AutoflattenRunSummary | null
  loading: boolean
  error: string | null
  onClose: () => void
}) {
      ...
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ fontSize: 14, fontWeight: 700, fontFamily: 'monospace', color: 'var(--text-primary)' }}>
            {detail ? `${detail.subject} — ${detail.run_id}` : loading ? 'Loading log…' : 'Log'}
          </div>
          ...
          <>
            ...
            <pre style={logPre}>
              {detail.log_tail && detail.log_tail.length > 0
                ? detail.log_tail
                : '(log empty — in-process runs write to the server log, not to stdout.log)'}
            </pre>
          </>
        )}
      </div>
...
export function AutoflattenInFlightRuns() {
  const [runs, setRuns] = useState<AutoflattenRunSummary[]>([])
  ...
  async function reload() {
    setLoading(true)
    try {
      const list = await fetchAutoflattenRunsList(true)
      setRuns(list)
    } finally {
      setLoading(false)
    }
  ...
  async function openLog(runId: string) {
    ...
    setLogLoading(true)
    try {
      const detail = await fetchAutoflattenRun(runId)
      setLogDetail(detail as unknown as AutoflattenRunSummary)
    } catch (e) {
  ...
  return (
    <div style={panelStyle}>
      ...
      {runs.length === 0 && (
        <div style={emptyStyle}>No autoflatten runs yet.</div>
      )}
      {runs.map((r) => {
