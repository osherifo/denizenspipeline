...
describe('analysis graph store', () => {
  ...
  it('starts group and study graphs with their fan-out and collector nodes', () => {
    get().newGraph('group')
    expect(get().graph.scope).toBe('group')
    expect(get().graph.nodes.map((n) => n.type)).toEqual(['control:map_subjects'])
    get().newGraph('study')
    expect(get().graph.nodes.map((n) => n.type)).toEqual(['control:study_groups'])
  })

  it('follows a group run: subjects, fan-out stages, and ignores nodes inside subject runs', async () => {
    await get().loadTemplate('group_mean')
    get().applyRunEvent({ event: 'group_subject_start', subject: 'S1', group: 'g' })
    get().applyRunEvent({ event: 'group_stage_start', stage: 'subject_fanout', group: 'g' })
    get().applyRunEvent({ event: 'node_start', node_id: 'mean', subject: 'S1' })
    expect(get().subjectStatus).toEqual({ S1: 'running' })
    expect(get().runStatus.subjects.status).toBe('running')
    expect(get().runStatus.mean).toBeUndefined()
    get().applyRunEvent({ event: 'group_subject_done', subject: 'S1', status: 'ok' })
    get().applyRunEvent({ event: 'group_stage_done', stage: 'subject_fanout', elapsed: 3 })
    get().applyRunEvent({ event: 'node_done', node_id: 'mean', elapsed: 1 })
    expect(get().subjectStatus.S1).toBe('ok')
    expect(get().runStatus.subjects).toEqual({ status: 'ok', durationS: 3 })
    expect(get().runStatus.mean.status).toBe('ok')
  })

  it('opens a subject graph body and comes back to the group graph', async () => {
    await get().loadTemplates()
    await get().loadTemplate('group_mean')
    await get().openBody('analyze')
    expect(get().graph.name).toBe('analyze')
    expect(get().stack.map((f) => f.label)).toEqual(['group_mean'])
    get().closeBody()
