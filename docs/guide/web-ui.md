...
- **The side panel** edits the selected node: what feeds each input, its parameters, the steps of the
  pipeline preparer, and extra section keys. Its **implementation** list swaps the node for another module of
  the same kind, such as another model or response loader, or precomputed features instead of an extractor.
  Only modules whose ports fit the node's connections are listed. The connections stay, and params with the
  same name carry over. A template is therefore a set of stages with a default module in each, and you pick
  the implementations you want. For the feature bundle, the order of the incoming edges is
