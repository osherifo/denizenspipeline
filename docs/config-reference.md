...
| `subject` | string | **yes** | — | Subject ID (e.g. `AN`). |
...
| Key | Type | Required | Default | Description |
|-----|------|----------|---------|-------------|
| `surface` | string | yes (for response loading) | — | Pycortex surface name (e.g. `ANfs`). |
| `transform` | string | yes (for response loading) | — | Pycortex transform name (e.g. `ANfs_default`). |
...
| `phact_hdf` | PHACT-style split HDF files: `subject{subject}_{modality}_fmri_data_{phase}.hdf`, each containing story datasets. Handles multi-repetition (3-D) arrays by collapsing across reps. |
...
PHACT HDF files store stories as `story_01`–`story_11` while the stimulus
...
split:
  test_runs: [wheretheressmoke]   # use the mapped names everywhere
```
